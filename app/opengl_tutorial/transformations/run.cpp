...
int main() {
    ...
    KeyManager keyManager = KeyManager(window, set<int> {
        GLFW_KEY_ESCAPE, GLFW_KEY_W, GLFW_KEY_S, GLFW_KEY_A, GLFW_KEY_D, GLFW_KEY_Q, GLFW_KEY_E
    });
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        Matrix transform = Matrix::translation(pos_x, pos_y, 0.0f) * Matrix::rotZ(rot) * Matrix::scale(.5, .5, .5);

        shader.use();
        shader.setInt("texture1", 0);
        shader.setInt("texture2", 1);
        shader.setFloat("mixAmt", 0.5f);
        shader.setMatrix2("transform", transform);
